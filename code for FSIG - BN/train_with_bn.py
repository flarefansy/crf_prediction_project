...
def full_connection(inputs, in_size, out_size, activation_func=None, norm=False):
    with tf.name_scope('layer'):
        with tf.name_scope('weights'):
            Weights = tf.Variable(tf.random_normal([in_size, out_size], mean=0., stddev=0.01))
        with tf.name_scope('biases'):    
            biases = tf.Variable(tf.zeros([out_size]))
            Wx_plus_b = tf.matmul(inputs, Weights) + biases
#    fc = tf.layers.dense(inputs, out_size)
    # Batch normalization
    # similar as:
    #  1) Wx_plus_b = (Wx_plus_b-fc_mean)/tf.sqrt(fc_var+epsilon)
    #  2) Wx_plus_b = Wx_plus_b*scale + shift
    if norm is True:
        scale = tf.Variable(tf.ones([out_size]),name='scale')
        shift = tf.Variable(tf.zeros([out_size]),name='shift')
        fc_mean = tf.Variable(tf.zeros([out_size]),name='mean')
        fc_var = tf.Variable(tf.zeros([out_size]),name='var')
        
        bat_mean, bat_var = tf.nn.moments(Wx_plus_b, axes=[0])
        fc_mean = tf.assign(fc_mean,bat_mean)
        fc_var = tf.assign(fc_var,bat_var)
...
prediction = full_connection(xs, 13, 3, activation_func=None, norm=True)
...
#    mean1, var1  = sess.run([fc_mean1, fc_var1], feed_dict={xs: train_x})
...
#        prediction_abc  = sess.run(prediction, feed_dict={xs: train_x})
#        me,va = sess.run([mean, var], feed_dict={xs: train_x})
